...
static void Tensor_from_ndarray_tol(Tensor& self,
                                    py::array_t<scalar_type> in_array,
...
static scalar_type Tensor_dot(const Tensor& self,
...
static std::shared_ptr<Tensor> Tensor_transpose_2(const Tensor& self,
                                                  py::tuple axes) {
...
static void Tensor_symmetrise_to(const Tensor& self,
                                 std::shared_ptr<Tensor> other,
                                 py::list permutations) {
    self.symmetrise_to(other, parse_permutations(permutations));
}

static void Tensor_antisymmetrise_to(const Tensor& self,
                                     std::shared_ptr<Tensor> other,
                                     py::list permutations) {
...
static std::shared_ptr<Tensor> add(std::shared_ptr<Tensor> a,
                                   std::shared_ptr<Tensor> b,
                                   std::shared_ptr<Tensor> out) {
    a->copy_to(out);
    out->add(b);
...
//
static std::shared_ptr<Tensor> Tensor_scalar__imul__(
      std::shared_ptr<Tensor> self, scalar_type number) {
    self->scale(number);
    return self;
}

static std::shared_ptr<Tensor> Tensor_scalar__mul__(
...
static std::shared_ptr<Tensor> Tensor__iadd__(
      std::shared_ptr<Tensor> self, const std::shared_ptr<Tensor>& other) {
    self->add(other);
    return self;
}

static std::shared_ptr<Tensor> Tensor__add__(
...
static std::shared_ptr<Tensor> Tensor__isub__(
      std::shared_ptr<Tensor> self, const std::shared_ptr<Tensor>& other) {
    self->add(-1.0, other);
    return self;
}

static std::shared_ptr<Tensor> Tensor__sub__(
      const std::shared_ptr<Tensor>& self,
      const std::shared_ptr<Tensor>& other) {
    auto cpy_ptr = self->copy();
...
static std::shared_ptr<Tensor> Tensor__mul__(
      const std::shared_ptr<Tensor>& self,
      const std::shared_ptr<Tensor>& other) {
...
static std::shared_ptr<Tensor> Tensor__truediv__(
      const std::shared_ptr<Tensor>& self,
      const std::shared_ptr<Tensor>& other) {
    auto out_ptr = self->nosym_like();
...
static std::shared_ptr<Tensor> Tensor__matmul__(
      const std::shared_ptr<Tensor>& self,
      const std::shared_ptr<Tensor>& other) {
...
void export_Tensor(py::module& m) {
    py::class_<Tensor, std::shared_ptr<Tensor>>(
          m, "Tensor",
          "Class representing the Tensor objects used for computations in "
          "adcman")
          ...
          .def("set_mask", &adcc::Tensor::set_mask,
               "Set all elements corresponding to an index mask, which is "
               "given by a "
          ...
          .def("to_ndarray", &Tensor_to_ndarray,
               "Export the tensor data to a standard np::ndarray by making a "
               "copy.")
          .def("set_from_ndarray", &Tensor_from_ndarray,
               "Set all tensor elements from a standard np::ndarray by making "
               "a copy. "
               "Provide an optional tolerance argument to increase the "
               "tolerance for the "
          ...
          .def("describe_symmetry", &Tensor::describe_symmetry,
               "Return a string providing a hopefully discriptive "
               "rerpesentation of the "
          ...
          .def("__mul__", &Tensor__mul__,
               "Multiply two tensors elementwise. Notice that this function "
               "discards any "
               "symmetry.")  // tensor * tensor
          .def("__truediv__", &Tensor__truediv__,
               "Divide two tensors elementwise. Notice that this function "
               "discards any "
               "symmetry.")                  // tensor / tensor
