...
parse_permutations(const py::iterable &permutations) {
...
static std::vector<size_t> convert_index_tuple(const ten_ptr &self,
                                               py::tuple idcs) {
  if (idcs.size() != self->ndim()) {
    throw py::value_error(
        "Number of elements passed in index tuple (== " +
        std::to_string(idcs.size()) +
        ") and dimensionality of tensor (== " + std::to_string(self->ndim()) +
        ") do not agree. Note, that at the moment any kind of slicing "
        "operation "
  ...
  for (size_t i = 0; i < idcs.size(); ++i) {
    ...
    try {
      ...
      throw py::cast_error(
          "Right now only integer indices are supported. Any kind of slicing "
          "operation "
          "(including partial slicing) are not yet implemented.");
    }
    if (idx < 0) {
      ...
      if (si > shape[i]) {
        throw py::index_error(
            "index " + std::to_string(idx) + " is out of bounds for axis " +
...
// Extra defs
//
static py::tuple Tensor_shape(const Tensor &self) {
...
static ten_ptr Tensor_from_ndarray_tol(
  ...
  if (nd < 1)
    throw invalid_argument("Cannot import from 0D array.");

  py::ssize_t pysize = 1;
  for (py::ssize_t i = 0; i < nd; ++i)
    pysize *= in_array.shape(i);
...
static py::array_t<scalar_type> Tensor_dot_list(const Tensor &self,
                                                py::list tensors) {
...
static ten_ptr Tensor_antisymmetrise_1(const Tensor &self,
...
static py::object tensordot_3(ten_ptr a, ten_ptr b) {
...
static ten_ptr Tensor_diagonal(ten_ptr ten, py::args permutations) {
  ...
  if (py::len(permutations) == 0) {
    ...
    for (auto itm : permutations)
      axes.push_back(itm.cast<size_t>());
  }
...
static ten_ptr linear_combination_strict(
  ...
  if (coefficients.ndim() != 1) {
    throw invalid_argument(
        "coefficients array needs to have exactly one dimension.");
  }
...
static py::list Tensor_select_n_min(const ten_ptr &self, size_t n) {
  ...
  py::list li;
  for (auto p : ret)
    li.append(py::make_tuple(p.first, p.second));
  return li;
}
...
static ten_ptr set_lt_scalar(const py::float_ n) {
  ...
  libtensor::btod_import_raw<1>(vector, bis.get_bis().get_dims())
      .perform(*v_ptr);
...
static ten_ptr Tensor_scalar__truediv__(const ten_ptr &self,
                                        scalar_type number) {
...
void export_Tensor(py::module &m) {
  py::class_<Tensor, std::shared_ptr<Tensor>>(
      ...
      .def(py::init(&make_tensor_zero),
           "Construct a Tensor object using a Symmetry object describing its "
           "symmetry "
           "properties.\n"
           "The returned object is not guaranteed to contain initialised "
           "memory. "
      ...
      .def_property_readonly(
          "needs_evaluation", &Tensor::needs_evaluation,
      ...
      .def("set_immutable", &Tensor::set_immutable,
           "Set the tensor as immutable, allowing some optimisations to be "
           "performed.")
      //
      ...
      .def("set_mask", &Tensor::set_mask,
           "Set all elements corresponding to an index mask, which is given by "
           "a "
      ...
      .def("set_from_ndarray", &Tensor_from_ndarray,
           "Set all tensor elements from a standard np::ndarray by making a "
           "copy. "
           "Provide an optional tolerance argument to increase the tolerance "
           "for the "
      ...
      .def("describe_symmetry", &Tensor::describe_symmetry,
           "Return a string providing a hopefully descriptive representation "
           "of the "
      ...
      .def("__setitem__", &Tensor__setitem__,
           "Set a tensor element or a slice of tensor elements. The operation "
           "will "
           "adhere symmetry, i.e. alter all elements equivalent by symmetry at "
           "once.")
      ...
      .def("select_n_max", &Tensor_select_n_max,
           "Select the n maximal elements.")
      .def("select_n_min", &Tensor_select_n_min,
      ...
      .def("__neg__",
           [](ten_ptr self) { return self->scale(-1.0); }) // - tensor
  ...
  m.def("linear_combination_strict", &linear_combination_strict,
        "coefficients"_a, "tensors"_a);
