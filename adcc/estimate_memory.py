...
def estimate_n_floats(method, n_o, n_c, n_v, max_subspace=0,
    ...
    sizes = {
        ...
        "cccc": (n_o**2 + n_o + 1) * (n_o**2 + n_o) / 8,
        "oocv": (n_o**2 + n_o) * n_c * n_v / 2,
        "ocov": n_o * n_c * n_o * n_v,
        "occv": n_o * n_c * n_c * n_v,
        "ocvv": n_o * n_c * (n_v**2 + n_v) / 2,
        "cccv": (n_c**2 + n_c) * n_c * n_v / 2,
        "ccvv": (n_c**2 + n_c) * (n_v**2 + n_v) / 4,
        "cvcv": (n_c * n_v + 1) * (n_c * n_v) / 2,
        "cvvv": n_c * n_v * (n_v**2 + n_v) / 2,
    }
