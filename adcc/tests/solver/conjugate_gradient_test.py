...
class TestConjugateGradient:
    def base_adc2(self, kind: str, guess_function: callable):
        ...
        matrix = adcc.AdcMatrix("adc2", testdata_cache.refstate("h2o_sto3g", case="gen"))
    ...
    def test_adc2_triplet_random(self):
        def guess_random(matrix, n_guesses):
            guess = adcc.guess_zero(matrix, spin_block_symmetrisation="antisymmetric")
            guess.set_random()
            return [guess]
    ...
    def test_adc1_linear_solve(self):
        conv_tol = 1e-9
        matrix = adcc.AdcMatrix("adc1", testdata_cache.refstate("h2o_sto3g", case="gen"))
        rhs = adcc.guess_zero(matrix)
        rhs.set_random()

        guess = rhs.copy()
        guess.set_random()
        res = conjugate_gradient(matrix, rhs, guess, callback=default_print, conv_tol=conv_tol)
        residual = matrix @ res.solution - rhs
        assert np.sqrt(residual @ residual) < conv_tol

    def test_adc2x_linear_solve(self):
        conv_tol = 1e-9
        matrix = adcc.AdcMatrix("adc2x", testdata_cache.refstate("h2o_sto3g", case="gen"))
        rhs = adcc.guess_zero(matrix)
        rhs.set_random()
        ...
        res = conjugate_gradient(
            matrix, rhs, guess, callback=default_print, conv_tol=conv_tol, Pinv=JacobiPreconditioner
        )
        residual = matrix @ res.solution - rhs
        assert np.sqrt(residual @ residual) < conv_tol
