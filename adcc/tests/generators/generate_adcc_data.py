...
_methods = {"pp": ("adc0", "adc1", "adc2", "adc2x", "adc3")}
_isr_orders = {
...
_small_cases_methods = {"pp": _methods["pp"] + ("adc4",)}
...
def generate_adc(
    ...
    n_singlets: int | None = None,
    n_triplets: int | None = None,
    n_spin_flip: int | None = None,
    ...
    **kwargs,
    ...
    _, kind = validate_state_parameters(
        hf, n_states=n_states, n_singlets=n_singlets, n_triplets=n_triplets, n_spin_flip=n_spin_flip
    )
    ...
    dump_excited_states(
        states, kind_group, only_full_mode=test_case.only_full_mode, dump_nstates=dump_nstates
    )
...
def generate_h2o_sto3g():
    ...
    states_per_case = {"cvs": 2, "fc-cvs": 2, "fv-cvs": 1, "fc-fv-cvs": 1}
    ...
    for method in _small_cases_methods["pp"]:
        ...
        for n_states in testcases.kinds_to_nstates(test_case.kinds[method.adc_type]):
            per_case = None
            if method.level.to_int() < 2:  # adc0/adc1
                per_case = {case: {n_states: n} for case, n in states_per_case.items()}
            n_states = {n_states: 3}
...
def generate_h2o_def2tzvp():
    ...
    test_case = testcases.get(n_expected_cases=1, name="h2o", basis="def2-tzvp").pop()
    generate_groundstate(test_case)
    for method in _methods["pp"]:
        ...
        for n_states in testcases.kinds_to_nstates(test_case.kinds[method.adc_type]):
            n_states = {n_states: 3}
            generate_adc_all(
                test_case, method=method, dump_nstates=2, states_per_case=None, **n_states
            )
            for isr_order in _isr_orders[method.level]:
                generate_adc_all(
                    test_case,
...
def generate_cn_sto3g():
    ...
    for method in _small_cases_methods["pp"]:
        ...
        for n_states in testcases.kinds_to_nstates(test_case.kinds[method.adc_type]):
            kwargs = {n_states: 3}
            generate_adc_all(
                test_case=test_case, method=method, dump_nstates=2, states_per_case=None, **kwargs
            )
            for isr_order in _isr_orders[method.level]:
                generate_adc_all(
                    test_case,
