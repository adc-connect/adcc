...
def run_psi4_tdscf(
    ...
    res = tdscf_excitations(wfn, states=5, triplets="NONE", tda=True, r_convergence=1e-7)

    # remove cavity files from PSI4 PCM calculations
    if pcm_options:
        for cavityfile in Path.cwd().iterdir():
            if cavityfile.name.startswith(("cavity.off_", "cavity.npz", "PEDRA.OUT_")):
                cavityfile.unlink()
    return wfn, res
...
def dump_results(
    ...
    ret["lr_excitation_energy"] = [round(float(r["EXCITATION ENERGY"]), 9) for r in res]
    ret["lr_osc_strength"] = [round(float(r["OSCILLATOR STRENGTH (LEN)"]), 5) for r in res]
...
def main():
    ...
    pcm_options = {"weight": 0.3, "pcm_method": "IEFPCM", "neq": True, "solvent": "Water"}
    psi4_results = {}
    for case in cases:
