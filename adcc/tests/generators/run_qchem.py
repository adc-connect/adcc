    ...
    **kwargs,
    ...
    if kwargs.get("gs_density_order", None) is not None:
        if method.level.to_int() < 3:
            raise ValueError("gs_density_order is only available for ADC(n) with n > 2")
    ...
    with tempfile.TemporaryDirectory(prefix=test_case.file_name, dir=Path.cwd()) as tmpdir:
        ...
        if run_qchem_scf:
            ...
            with open_pyscf_result(test_case) as hdf5_file:
                ...
                generate_qchem_savedir(
                    pyscf_data=hdf5_file,
                    savedir=savedir,
                    core_orbitals=n_core_orbitals,
                    frozen_core=n_frozen_core,
                    frozen_virtual=n_frozen_virtual,
                )
                savedir = savedir.name
                # extrac the relevant data from the pyscf data for the infile.
                args["basis_definition"] = _extract_dataset(hdf5_file["qchem_formatted_basis"])[1]
        ...
        generate_qchem_input_file(
            ...
            any_states=n_states,
            singlet_states=n_singlets,
            triplet_states=n_triplets,
            sf_states=n_spin_flip,
            run_qchem_scf=run_qchem_scf,
            **args,
        )
        # call qchem and wait for completion
        execute_qchem(
            infile=infile.name, outfile=outfile.name, savedir=savedir, workdir=str(tmpdir.resolve())
        )
        # after the calculation we should have the context file in the tmpdir
        context_file = tmpdir / _qchem_context_file
        if not context_file.exists():
            raise FileNotFoundError(
                f"Expected the qchem data in {context_file} after the qchem calculation."
            )
        ...
        with h5py.File(context_file, "r") as context:
            ...
            gs_data = None
            if import_gs:
                gs_data = import_groundstate(context, only_full_mode=test_case.only_full_mode)
...
def open_pyscf_result(test_case: testcases.TestCase) -> h5py.File:
    ...
    if not hdf5_file.exists():
        raise FileNotFoundError(
            "Could not find hdf5 file with pyscf HF data "
...
def generate_qchem_savedir(
    pyscf_data: h5py.File,
    savedir: str | Path,
    ...
    savedir_writer.write(
        ...
        ao_density_aa=ao_density_aa,
        ao_density_bb=ao_density_bb,
        eri_blocks=eri_blocks,
        ao_integrals=ao_integrals,
        purecart=purecart,
    )


def build_antisym_eri(
    pyscf_data: h5py.File,
    core_orbitals: int | None = None,
    frozen_core: int | None = None,
    frozen_virtual: int | None = None,
...
def determine_purecart(pyscf_data: h5py.File) -> int:
    ...
    """
    _, use_cart_angular_funcs = _extract_dataset(pyscf_data["cartesian_angular_functions"])
    return 2222 if use_cart_angular_funcs else 1111


def execute_qchem(infile: str, outfile: str, workdir: str, savedir: str | None = None) -> None:
    """
    ...
    if savedir is None:
        code = subprocess.run(["qchem", infile, outfile], env=env, check=False).returncode
    else:
        code = subprocess.run(["qchem", infile, outfile, savedir], env=env, check=False).returncode
    # qchem did not terminate normal -> copy the outputfile to the working directory
    if code:
        if Path(outfile).exists():
            shutil.copy(outfile, cwd)
        raise RuntimeError(
            "Qchem calculation did finish with return code != 0. "
...
def generate_qchem_input_file(
    infile: str | Path,
    adc_method: AdcMethod,
    basis: str,
    xyz: str,
    charge: int,
    multiplicity: int,
    ...
    bohr: bool = True,
    any_states: int = 0,
    singlet_states: int = 0,
    triplet_states: int = 0,
    sf_states: int = 0,
    maxiter: int = 160,
    conv_tol: int = 10,
    ...
    scf_options = ["use_libqints             true", "gen_scfman               true"]
    if run_qchem_scf:
        export_test_data = 2
    else:
        export_test_data = 42
        scf_options.extend(["scf_guess                read", "max_scf_cycles           0"])
...
_isr_order_dict: dict[int | str, str] = {3: "3", "3d": "THREE_D"}

_gs_density_order_dict: dict[str, str] = {"sigma4+": "sigma_4_plus"}
