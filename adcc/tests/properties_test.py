...
cases = [
    ...
    for kind in ["singlet", "any", "spin_flip"]
    if kind in case.kinds.pp
]
unrestricted_cases = [
    (case.file_name, "gen", kind)
    for case in test_cases
    if not case.restricted
    for kind in ["singlet", "any", "spin_flip"]
    if kind in case.kinds.pp
]
...
class TestProperties:
    ...
    def test_transition_dipole_moments(
        self, system: str, case: str, kind: str, adc_method: str, generator: str, isr_order=None
    ):
        ...
        state = testdata_cache._make_mock_adc_state(
            system=system, method=adc_method, case=case, kind=kind, source=generator
        )
        ...
        if generator == "adcman" and kind == "triplet":
            ref_tdms = [np.array([0.0, 0.0, 0.0]) for _ in range(len(refevals))]
        else:
    ...
    def test_oscillator_strengths(
        ...
        for i in range(n_ref):
            ...
            assert res_oscs[i] == pytest.approx(2.0 / 3.0 * ref_tdm_norm * refevals[i])

    @pytest.mark.parametrize("generator", generators)
    @pytest.mark.parametrize("system,case,kind", cases)
    def test_state_dipole_moments(
        self, system: str, case: str, kind: str, adc_method: str, generator: str, isr_order=None
    ):
    ...
    @pytest.mark.parametrize(
        "system,case,kind", [c for c in unrestricted_cases if "cvs" not in c[1]]
    )
    def test_state_ssq(self, system: str, case: str, kind: str, adc_method: str, isr_order=None):
    ...
    @pytest.mark.parametrize("generator", generators)
    @pytest.mark.parametrize("system,case,kind", [c for c in cases if "cvs" not in c[1]])
    def test_state2state_transition_dipole_moments(
        self, system: str, case: str, kind: str, adc_method: str, generator: str, isr_order=None
    ):
        ...
        for i in range(len(state.excitation_vector) - 1):
            ...
            for ii, j in enumerate(range(i + 1, state.size)):
                ...
                assert_allclose_signfix(
                    state2state.transition_dipole_moment[ii], fromi_ref[ii], atol=1e-4
                )

    @pytest.mark.parametrize("case", ["gen", "cvs"])
    def test_magnetic_transition_dipole_moments_z_component(
        self, adc_method: str, case: str, isr_order=None
    ):
        ...
        if "cvs" in case:
            ...
            state = run_adc(scfres, method=adc_method, n_singlets=5, core_orbitals=2)
        else:
    ...
    @pytest.mark.parametrize("system,case,kind", cases)
    def test_magnetic_transition_dipole_moments(
        self, system: str, case: str, kind: str, adc_method: str, isr_order=None
    ):
    ...
    # Only adcc reference data available.
    @pytest.mark.parametrize("system,case,kind", cases)
    def test_transition_dipole_moments_velocity(
        self, system: str, case: str, kind: str, adc_method: str, isr_order=None
    ):
        ...
        for i in range(n_ref):
            assert_allclose_signfix(
                res_dms[i], refdata["transition_dipole_moments_velocity"][i], atol=1e-4
            )

    # Only adcc reference data available.
    @pytest.mark.parametrize("system,case,kind", cases)
    def test_transition_quadrupole_moments(
        self, system: str, case: str, kind: str, adc_method: str, isr_order=None
    ):
        ...
        for g_origin in gauge_origins:
            ...
            for i in range(n_ref):
                assert_allclose_signfix(
                    res_dms[i], refdata[f"transition_quadrupole_moments_{g_origin}"][i], atol=1e-4
                )

    # Only adcc reference data available.
    @pytest.mark.parametrize("system,case,kind", cases)
    def test_rotatory_strengths(
        self, system: str, case: str, kind: str, adc_method: str, isr_order=None
    ):
