...
formaldehyde = testcases.get_by_filename("formaldehyde_sto3g", "formaldehyde_ccpvdz")
...
class TestPCM:
    ...
    pcm_options: ClassVar = {
        "psi4": {"pcm_method": "IEFPCM", "solvent": "Water"},
    ...
    def test_pcm_ptlr(self, system, case, method, backend):
        ...
        scfres = run_hf(
            ...
            conv_tol_grad=1e-11,
            max_iter=150,
            pcm_options=self.pcm_options[backend],
        ...
        assert_allclose(state.excitation_energy, result["lr_excitation_energy"], atol=5e-3)

        # Consistency check with values obtained with ADCc
        assert_allclose(state.excitation_energy, result["ptlr_adcc_excitation_energy"], atol=1e-5)
    ...
    def test_pcm_linear_response(self, system, case, method, backend):
        ...
        state = adcc.run_adc(
            matrix, n_singlets=5, conv_tol=1e-7, max_iter=400, max_subspace=25, environment=False
        )
        assert state.converged
        assert_allclose(
            state.excitation_energy_uncorrected, result["lr_excitation_energy"], atol=1e-5
        )

        state_cis = adcc.ExcitedStates(state, property_method="isr0")
        assert_allclose(state_cis.oscillator_strength, result["lr_osc_strength"], atol=1e-3)
...
class TestPCMcomparison(unittest.TestCase):
    def test_comparison_dd_pcmsolver(self):
        ...
        scfres_ief = psi4_run_pcm_hf(
            **scfargs,
            options=dict(pcm=True, **psiopts),
            pcm_options={"pcm_method": "IEFPCM", "solvent": "Water", "radiiset": "UFF"},
        )
        state_ief = adcc.run_adc(scfres_ief, **adcopts)
        assert state_ief.converged

        scfres_dd = psi4_run_pcm_hf(
            **scfargs,
            options=dict(
                ddx=True,
                ddx_model="pcm",
    ...
    def test_comparison_gaussian(self):
        ...
        state = adcc.cis(scfres, n_singlets=7, conv_tol=1e-7, environment="linear_response")
        assert state.converged

        eV = constants.value("Hartree energy in eV")
        ref_gaussian = np.array([9.2735, 11.2083, 11.2083, 15.2852, 15.2852, 17.6378, 17.9519]) / eV
