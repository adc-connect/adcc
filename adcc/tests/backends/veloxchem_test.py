...
@pytest.mark.skipif(not have_backend("veloxchem"), reason="Veloxchem not found.")
class TestVeloxchem:
    def base_test(self, scfdrv):
        ...
        if hfdata.restricted:
            ...
            assert_array_equal(hfdata.orben_f[:n_orbs_alpha], hfdata.orben_f[n_orbs_alpha:])

            n_alpha = int(scfdrv.task.molecule.number_of_electrons() / 2)
            n_beta = n_alpha
            mo_energy = (scfdrv.mol_orbs.ea_to_numpy(), scfdrv.mol_orbs.ea_to_numpy())
        else:
            assert hfdata.n_alpha >= hfdata.n_beta
            ...
            n_alpha = np.sum(scfdrv.task.molecule.get_aufbau_alpha_occupation(n_mo) > 0)
            n_beta = np.sum(scfdrv.task.molecule.get_aufbau_beta_occupation(n_mo) > 0)
            mo_energy = (scfdrv.mol_orbs.ea_to_numpy(), scfdrv.mol_orbs.eb_to_numpy())
    ...
    def operators_test(self, scfdrv):
        ...
        dipole_mats = dipole_drv.compute(scfdrv.task.molecule, scfdrv.task.ao_basis)
        integrals = (
            -1.0 * dipole_mats.x_to_numpy(),
            -1.0 * dipole_mats.y_to_numpy(),
            -1.0 * dipole_mats.z_to_numpy(),
        )
        operator_import_from_ao_test(scfdrv, integrals, operator="electric_dipole")

        # Test electric dipole velocity
        linmom_drv = LinearMomentumIntegralsDriver(scfdrv.task.mpi_comm)
        linmom_mats = linmom_drv.compute(scfdrv.task.molecule, scfdrv.task.ao_basis)
        integrals = (
            -1.0 * linmom_mats.x_to_numpy(),
            -1.0 * linmom_mats.y_to_numpy(),
            -1.0 * linmom_mats.z_to_numpy(),
        )
        operator_import_from_ao_test(scfdrv, integrals, operator="electric_dipole_velocity")
        ...
        for gauge_origin in gauge_origins:
            ...
            angmom_mats = angmom_drv.compute(scfdrv.task.molecule, scfdrv.task.ao_basis)
            integrals = (
                0.5 * angmom_mats.x_to_numpy(),
                0.5 * angmom_mats.y_to_numpy(),
                0.5 * angmom_mats.z_to_numpy(),
            )
            operator_import_from_ao_test(scfdrv, integrals, "magnetic_dipole", gauge_origin)
