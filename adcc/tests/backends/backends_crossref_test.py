...
methox = testcases.get_by_filename("r2methyloxirane_sto3g", "r2methyloxirane_ccpvdz")
...
@pytest.mark.skipif(
    len(backends) < 2, reason="Need at least two available backends for cross reference test."
)
class TestCrossReferenceBackends:
    ...
    def test_adc2_h2o(self, system, case):
        ...
        if len(backends_test) < 2:
            pytest.skip("Veloxchem does not support f-functions. Not enough backends available.")
    ...
    def test_adc2_r2methyloxirane(self, system, case):
        ...
        if len(backends_test) < 2:
            pytest.skip(
                "Veloxchem not available for (R)-2-Methyloxirane. Not enough backends available."
            )
        method = "cvs-adc2" if "cvs" in case else "adc2"
        core_orbitals = system.core_orbitals if "cvs" in case else None
        frozen_core = system.frozen_core if "fc" in case else None
    ...
    for comb in itertools.combinations(results, r=2):
        ...
        if len(set(comb) & set(backends_with_gauge_origin)) == 2:
            ...
            for gauge_origin in gauge_origins:
                assert_allclose(
                    refstate1.gauge_origin_to_xyz(gauge_origin),
                    refstate2.gauge_origin_to_xyz(gauge_origin),
                    atol=1e-5,
                )

        assert_allclose(refstate1.nuclear_total_charge, refstate2.nuclear_total_charge, atol=atol)
        assert_allclose(refstate1.nuclear_dipole, refstate2.nuclear_dipole, atol=atol)

        if (
            "electric_dipole" in refstate1.operators.available
            and "electric_dipole" in refstate2.operators.available
        ):
            assert_allclose(refstate1.dipole_moment, refstate2.dipole_moment, atol=atol)
...
def compare_adc_results(adc_results, atol):
    for comb in itertools.combinations(adc_results, r=2):
        ...
        assert_allclose(state1.excitation_energy, state2.excitation_energy)
        ...
        for v1, v2 in zip(state1.excitation_vector, state2.excitation_vector):
            for block in blocks1:
                ...
                assert_allclose(
                    ...
                    np.abs(v2np),
                    atol=10 * atol,
                    err_msg=f"ADC vectors are not equalin block {block}",
                )

        def fix_signs(actual, desired, atol):
            fixed_signs = np.sign(actual * (np.absolute(actual) > atol)) * np.sign(
                desired * (np.absolute(desired) > atol)
            )
        ...
        # all properties
        if (
            "electric_dipole" in state1.operators.available
            and "electric_dipole" in state2.operators.available
        ):
            assert_allclose(state1.oscillator_strength, state2.oscillator_strength, atol=atol)
            assert_allclose(state1.state_dipole_moment, state2.state_dipole_moment, atol=atol)
            ...
            fixed_signs_new = fix_signs(
                state1.transition_dipole_moment, state2.transition_dipole_moment, atol=atol
            )
        ...
        if (
            "electric_dipole_velocity" in state1.operators.available
            and "electric_dipole_velocity" in state2.operators.available
        ):
            assert_allclose(
                state1.oscillator_strength_velocity, state2.oscillator_strength_velocity, atol=atol
            )
        ...
        if (
            "magnetic_dipole" in state1.operators.available
            and "magnetic_dipole" in state2.operators.available
        ):
        ...
        if (
            "electric_quadrupole" in state1.operators.available
            and "electric_quadrupole" in state2.operators.available
        ):
        ...
        if (
            ...
            and "overlap" in state1.operators.available
            and "overlap" in state2.operators.available
        ):
        ...
        has_rotatory1 = all(
            ...
            for op in ["magnetic_dipole", "electric_dipole_velocity"]
        )
        has_rotatory2 = all(
        ...
        if has_rotatory1 and has_rotatory2:
            assert_allclose(state1.rotatory_strength, state2.rotatory_strength, atol=atol)

        has_rotatory_len1 = all(
            op in state1.operators.available for op in ["electric_dipole", "magnetic_dipole"]
        )
        has_rotatory_len2 = all(
            op in state2.operators.available for op in ["electric_dipole", "magnetic_dipole"]
        )
        if has_rotatory_len1 and has_rotatory_len2:
            for gauge_origin in gauge_origins:
                ...
                if gauge_origin == "mass_center":
                    ...
                    atol_tdm_mag = atol
                assert_allclose(
                    state1.rotatory_strength_length(gauge_origin),
