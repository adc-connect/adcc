...
cases = [
    (case.file_name, c, kind) for case in test_cases for c in ["gen"] for kind in case.kinds.pp
]
...
class TestStateDiffDm:
    def calculate_adcn_excitation_energy(self, state):
        ...
        if level - 1 >= 0:
            method_order_minus_one = method.at_level(level - 1)
        ...
        for es in range(n_states):
            ...
            if method_order_minus_one is not None:
                ...
                for block in dens_1p.blocks:
                    ...
                    eri_1p = np.einsum("piqi->pq", hf.eri(f"{s1}o1{s2}o1").to_ndarray())
                    excitation_energy[es] -= np.einsum(
                        "pq,pq->", eri_1p, dens_1p[block].to_ndarray()
                    )
                ...
                np.testing.assert_allclose(dens_1p.oo.to_ndarray(), d_oo, atol=1e-14)
                d_ov = (
                    1
                    / (n_occ - 1)
                    * (
                ...
                np.testing.assert_allclose(dens_1p.ov.to_ndarray(), d_ov, atol=1e-14)
                d_vo = (
                    1
                    / (n_occ - 1)
                    * (
                ...
                np.testing.assert_allclose(dens_1p.vo.to_ndarray(), d_vo, atol=1e-14)
                d_vv = (
                    1
                    / (n_occ - 1)
                    * (
                ...
                np.testing.assert_allclose(dens_1p.vv.to_ndarray(), d_vv, atol=1e-14)
        return excitation_energy

    def test_adcn(self, adc_method: str, system: str, case: str, kind: str):
        state = testdata_cache.adcc_states(
            system=system, method=adc_method, kind=kind, case=case, isr_order=None
        )
