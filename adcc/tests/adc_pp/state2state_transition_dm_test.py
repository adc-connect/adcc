...
from adcc import run_adc, State2States
...
class TestState2StateTransitionDm:
    def test_adcn(self, method: str, case: str):
        ...
        for i in range(3):  # three excited states
            s2s = State2States(state, initial=i)
            prefac = 1.0 / (n_electrons - 1)
            state_tdm_2p = s2s.transition_dm_2p
            state_tdm = s2s.transition_dm
            # final state is higher than inital state
            for es in range(3 - i - 1):
