...
def s2s_tdm_ip_adc2(mp, amplitude_l, amplitude_r, intermediates):
    ...
    t_ia =  (- (
            + einsum("jkba,jkbi->ia", t2, hf.oovo) 
            + einsum("jibc,jbac->ia", t2, hf.ovvv))
            / (2 * (direct_sum("-i+a->ia", 
                    hf.foo.diagonal(), hf.fvv.diagonal())))) 
    p0_oo = - 0.5 * einsum("ikab,jkab->ij", t2, t2)
    p0_ov = t_ia
...
#    ru1 = einsum("i,ijab->jab", u1, t2).evaluate()
    ...
    dm.oo = (  # ip_adc2_p_oo
        + p1_oo + p2_oo 
        - 0.5 * einsum("k,jk,i->ij", ul1, p0_oo, ur1)
        - 0.5 * einsum("j,ki,k->ij", ul1, p0_oo, ur1)
    ...
    dm.ov = (  # ip_adc2_p_ov
        + p_ov
        + 1/sqrt(2) * einsum("klb,klba,i->ia", ul2, t2, ur1)
        + sqrt(2) * einsum("kb,ikba->ia", einsum("klb,l->kb", ul2, ur1), t2)
        - einsum("k,ka,i->ia", ul1, p0_ov, ur1)
    )
    
    dm.vo = (  # ip_adc2_p_vo
        + p_vo
        + 1/sqrt(2) * einsum("i,klba,klb->ai", ul1, t2, ur2)
        + sqrt(2) * einsum("lb,liba->ai", einsum("k,klb->lb", ul1, ur2), t2)
        - einsum("k,ka,i->ai", ur1, p0_ov, ul1) 
