...
class OperatorIntegrals:
    ...
    @cached_property
    @timed_member_call("_import_timer")
    def d_eri_dB_1p(self) -> tuple[OneParticleOperator, ...]:
        """Return the differentiated ERI with respect to the magnetic field
    ...
    def eri_1p(self, hf) -> OneParticleOperator:
    ...
    def magnetic_dipole_giao_1p(
        ...
        for comp in range(3):  # xyz
            mag_dip_comp = OneParticleOperator(
                self.mospaces, symmetry=OperatorSymmetry.ANTIHERMITIAN
            )

            # this minus sign is to go from m = - 0.5 l and l is 2 * d<E>/dB.
            # -> only one minus sign
            mag_dip_comp.vv = -1.0 * (
                # N^3: V^3 / N^2: V^2
                -2 * einsum("cb,ac->ab", d_T_dB[comp].vv, hf.fvv).antisymmetrise()
                # N^2: V^2 / N^2: V^2
                + d_h_dB[comp].vv
                + d_eri_dB_1p[comp].vv
            )
            ...
            mag_dip_comp.oo = -1.0 * (
                # N^3: O^3 / N^2: O^2
                -2 * einsum("kj,ik->ij", d_T_dB[comp].oo, hf.foo).antisymmetrise()
                # N^2: O^2 / N^2: O^2
                + d_h_dB[comp].oo
                + d_eri_dB_1p[comp].oo
            )
            ...
            mag_dip_comp.ov = -1.0 * (
                # why do I have here the minus?
                # N^3: O^2V^1 / N^2: O^1V^1
                - einsum("ja,ij->ia", d_T_dB[comp].ov, hf.foo)
                # N^3: O^1V^2 / N^2: V^2
                + einsum("bi,ab->ia", d_T_dB[comp].vo, hf.fvv)
                # N^2: O^1V^1 / N^2: O^1V^1
                + d_h_dB[comp].ov
                + d_eri_dB_1p[comp].ov
            )
    ...
    @cached_member_function(timer="_import_timer", separate_timings_by_args=True)
    def magnetic_dipole_giao_2p_1p(self, hf, gauge_origin: str = "origin"
                                   ) -> tuple[OneParticleOperator, ...]:
        d_T_dB = self.d_natural_connection_matrix_dB(gauge_origin)
        d_eri_dB_1p = self.d_eri_dB_1p
        ...
        for comp in range(3):  # xyz
            ...
            mag_dip_comp.vv = -1.0 * (
                2 * (
                    ...
                    + 1 * einsum("iajb,ji->ab", hf.ovov, d_T_dB[comp].oo)
                    # N^3: V^3 / N^2: V^2
                    + 1 * einsum("cb,ac->ab", d_T_dB[comp].vv, eri_1p.vv)
                    # N^3: O^1V^2 / N^2: V^2
                    + 1 * einsum("ib,ia->ab", d_T_dB[comp].ov, eri_1p.ov)
                ).antisymmetrise()
                # N^2: V^2 / N^2: V^2
                - d_eri_dB_1p[comp].vv
            )
            ...
            mag_dip_comp.oo = -1.0 * (
                2 * (
                    # N^4: O^4 / N^4: O^4
                    + 1 * einsum("ikjl,lk->ij", hf.oooo, d_T_dB[comp].oo)
                    # N^3: O^2V^1 / N^2: O^1V^1
                    + 1 * einsum("aj,ia->ij", d_T_dB[comp].vo, eri_1p.ov)
                    # N^3: O^3 / N^2: O^2
                    + 1 * einsum("kj,ik->ij", d_T_dB[comp].oo, eri_1p.oo)
                ).antisymmetrise()
                # N^2: O^2 / N^2: O^2
                - d_eri_dB_1p[comp].oo
            )
            ...
            mag_dip_comp.ov = -1.0 * (
                ...
                + 1 * einsum("ijka,jk->ia", hf.ooov, d_T_dB[comp].oo)
                # N^4: O^3V^1 / N^4: O^3V^1
                + 1 * einsum("ba,ib->ia", d_T_dB[comp].vv, eri_1p.ov)
                # N^3: O^2V^1 / N^2: O^1V^1
                + 1 * einsum("ja,ij->ia", d_T_dB[comp].ov, eri_1p.oo)
                # N^4: O^3V^1 / N^4: O^3V^1
                - 1 * einsum("ikja,jk->ia", hf.ooov, d_T_dB[comp].oo)
                # N^3: O^1V^2 / N^2: V^2
                - 1 * einsum("bi,ab->ia", d_T_dB[comp].vo, eri_1p.vv)
                # N^3: O^2V^1 / N^2: O^1V^1
                - 1 * einsum("ji,ja->ia", d_T_dB[comp].oo, eri_1p.ov)
                # N^2: O^1V^1 / N^2: O^1V^1
                - d_eri_dB_1p[comp].ov
            )
            mag_dip.append(mag_dip_comp)
    ...
    def d_natural_connection_matrix_dB(
        ...
        return self._import_g_origin_dep_dip_like_operator(
            integral="d_natural_connection_matrix_dB", gauge_origin=gauge_origin,
            symmetry=OperatorSymmetry.NOSYMMETRY
        )
