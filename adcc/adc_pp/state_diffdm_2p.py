...
def diffdm_adc0_2p(mp, amplitude, intermediates):
    ...
    dm.oooo = (
        ...
        + (
            ...
            + 8.0 * einsum("ik,jl->ijkl", p2_oo, d_oo)
    ...
    dm.ooov = (
        ...
        + (
            ...
            + 2.0 * einsum("ja,ik->ijka", p2_ov, d_oo)
        ).antisymmetrise(0, 1)
    )
    dm.ovov = (
        # N^4: O^2V^2 / N^4: O^2V^2
        ...
        + 2.0 * einsum("ab,ij->iajb", p2_vv, d_oo)
    )
    dm.ovvv = (
    ...
    dm.vvvv = (
        # N^6: O^2V^4 / N^4: V^4
        + 2.0 * einsum("ijab,ijcd->abcd", u2, u2)
    )
    return dm
...
def diffdm_adc1_2p(mp, amplitude, intermediates):
    ...
    dm.ooov += (
        # N^6: O^4V^2 / N^4: O^2V^2
        + 1.0 * einsum("ijkl,la->ijka", ru2_oooo, u1)
        # N^5: O^3V^2 / N^4: O^2V^2
        - 1.0 * einsum("kb,ijab->ijka", p2_ov, t2)
        + (
            # N^5: O^3V^2 / N^4: O^2V^2
            + 2.0 * einsum("jk,ia->ijka", ru2_oo, u1)
            # N^6: O^4V^2 / N^4: O^2V^2
            - 4.0 * einsum("jklb,ilab->ijka", p2_ooov, t2)
            # N^5: O^3V^2 / N^4: O^2V^2
            + 2.0 * einsum("ja,ik->ijka",
                           einsum("jlmb,lmab->ja", p2_ooov, t2), d_oo)
            # N^4: O^2V^2 / N^4: O^2V^2
            + 2.0 * einsum("ia,jk->ijka",
                           einsum("lb,ilab->ia", p2_ov, t2), d_oo)
            # N^5: O^3V^2 / N^4: O^2V^2
            + 2.0 * einsum("ia,jk->ijka",
                           einsum("il,la->ia", ru2_oo, u1), d_oo)
        ).antisymmetrise(0, 1)
    )
    dm.oovv += (
        # N^4: O^2V^2 / N^4: O^2V^2
    ...
    dm.ovvv += (
        ...
        + 1.0 * einsum("ijka,jkbc->iabc", p2_ooov, t2)
        # N^5: O^2V^3 / N^4: O^1V^3
        - 1.0 * einsum("ja,ijbc->iabc", p2_ov, t2)
        + (
            # N^5: O^2V^3 / N^4: O^1V^3
            + 2.0 * einsum("ac,ib->iabc", ru2_vv, u1)
            # N^6: O^3V^3 / N^4:  O^1V^3
            - 4.0 * einsum("ikab,kc->iabc", ru2_oovv, u1)
        ).antisymmetrise(2, 3)
    )
    return dm
...
def diffdm_adc2_2p(mp, amplitude, intermediates):
    ...
    dm.oooo += (
        ...
        - 1 * einsum("klmc,ijmc->ijkl", ru1_ooov, ru1_ooov)
        + (
            # N^4: O^4 / N^4: O^4
            - 4.0 * einsum("il,jk->ijkl", p1_oo, p0.oo)
            # N^6: O^5V^1 / N^4: O^2V^2
            + 4.0 * einsum("jlmc,ikmc->ijkl", einsum("lb,jmbc->jlmc", u1, t2),
                           einsum("ia,kmac->ikmc", u1, t2))
            # N^4: O^2V^2 / N^4: O^2V^2
            + 4.0 * einsum("il,jk->ijkl", einsum("lc,ic->il", ru1, ru1), d_oo)
            # N^5: O^3V^2 / N^4: O^2V^2
            + 4.0 * einsum("ik,jl->ijkl",
                           einsum("kmnc,imnc->ik", einsum("nb,kmbc->kmnc", u1, t2),
                                  einsum("na,imac->imnc", u1, t2)), d_oo)
            # N^5: O^3V^2 / N^4: O^2V^2
        ...
        + (
            # N^5: O^3V^2 / N^4: O^2V^2
            + 2.0 * einsum("iklb,jb->ijkl", einsum("ic,klbc->iklb",
                                                   einsum("ma,imac->ic", u1, t2),
                                                   t2), u1)
            # N^6: O^4V^2 / N^4: O^2V^2
            - 1.0 * einsum("jklm,im->ijkl", einsum("jmbc,klbc->jklm", t2, t2),
                           p1_oo)
    ...
    dm.ooov += (
        ...
        # N^4: O^3V^1 / N^4: O^3V^1
        - 2.0 * einsum("jk,ia->ijka", p1_oo, p0.ov)
        # N^4: O^3V^1 / N^4: O^3V^1
        + 2.0 * einsum("ia,jk->ijka", einsum("il,la->ia",
                                             einsum("lb,ib->il", u1, p0.ov), u1),
                       d_oo)
        # N^4: O^3V^1 / N^4: O^3V^1
    ...
    dm.oovv += (
        + (
            # N^4: O^2V^2 / N^4: O^2V^2
            + 4.0 * einsum("ib,ja->ijab", einsum("kc,ikbc->ib", u1, td2), u1)
        ).antisymmetrise(0, 1).antisymmetrise(2, 3)
        + (
            # N^5: O^3V^2 / N^4: O^2V^2
            + 2.0 * einsum("ijka,kb->ijab", einsum("kc,ijac->ijka", u1, td2), u1)
        ).antisymmetrise(2, 3)
        + (
            # N^5: O^3V^2 / N^4: O^2V^2
            - 2.0 * einsum("jk,ikab->ijab", p1_oo, td2)
        ).antisymmetrise(0, 1)
    )
    dm.ovov += (
        # N^6: O^4V^2 / N^4: O^2V^2
        + 1 * einsum("iklb,jkla->iajb", einsum("ld,ikbd->iklb", u1, t2),
                     einsum("lc,jkac->jkla", u1, t2))
        # N^4: O^2V^2 / N^4: O^2V^2
        + 1 * einsum("ab,ij->iajb", p1_vv, p0.oo)
        # N^4: O^2V^2 / N^4: O^2V^2
        + 1 * einsum("ij,ab->iajb", p1_oo, p0.vv)
        # N^6: O^3V^3 / N^4: O^2V^2
        - 1 * einsum("jlac,ilbc->iajb", einsum("kl,jkac->jlac", p1_oo, t2), t2)
        # N^6: O^4V^2 / N^4: O^2V^2
        + 0.5 * einsum("ijla,lb->iajb", einsum("ijkl,ka->ijla",
                                               einsum("ikcd,jlcd->ijkl", t2, t2),
                                               u1), u1)
        # N^6: O^4V^2 / N^4: O^2V^2
        + 0.5 * einsum("jklb,ikla->iajb", einsum("jd,klbd->jklb", u1, t2),
                       einsum("ic,klac->ikla", u1, t2))
        # N^4: O^2V^2 / N^4: O^2V^2
        - 1 * einsum("ib,ja->iajb", einsum("kd,ikbd->ib", u1, t2),
                     einsum("lc,jlac->ja", u1, t2))
        # N^5: O^2V^3 / N^4: O^2V^2
        - 1 * einsum("ab,ij->iajb", einsum("lmad,lmbd->ab",
                                           einsum("km,klad->lmad", p1_oo, t2), t2),
                     d_oo)
        # N^4: O^2V^2 / N^4: O^2V^2
        - 1 * einsum("ab,ij->iajb",
                     einsum("lb,la->ab", einsum("kd,klbd->lb", u1, t2),
                            einsum("mc,lmac->la", u1, t2)), d_oo)
        # N^5: O^3V^2 / N^4: O^2V^2
        - 0.5 * einsum("ab,ij->iajb",
                       einsum("klmb,klma->ab", einsum("md,klbd->klmb", u1, t2),
                              einsum("mc,klac->klma", u1, t2)), d_oo)
        + (
            # N^5: O^3V^2 / N^4: O^2V^2
            + 2.0 * einsum("ijlb,la->iajb",
                           einsum("jc,ilbc->ijlb", einsum("kd,jkcd->jc", u1, t2),
                                  t2), u1)
            # N^5: O^3V^2 / N^4: O^2V^2
            ...
            + 2.0 * einsum("ikbc,jkac->iajb",
                           einsum("il,klbc->ikbc", p1_oo, t2), t2)
            # N^4: O^2V^2 / N^4: O^2V^2
    ...
    dm.ovvv += (
        ...
        # N^4: O^1V^3 / N^4: O^1V^3
        + 2.0 * einsum("ac,ib->iabc", p1_vv, p0.ov)
    ).antisymmetrise(2, 3)
    dm.vvvv += (
        # N^6: O^2V^4 / N^4: V^4
        + 1 * einsum("jkab,jkcd->abcd", einsum("ij,ikab->jkab", p1_oo, t2), t2)
        + (
            # N^4: V^4 / N^4: V^4
            + 4.0 * einsum("ac,bd->abcd", p1_vv, p0.vv)
            # N^6: O^3V^3 / N^4: V^4
