...
"""
    ...
    if kind == "spin_flip" and guesses is None:
        spin_change = -1
    elif matrix.type != "pp" and guesses is None:
        spin_change = -0.5 if matrix.type == "ip" else +0.5
        if not is_alpha:
            spin_change *= -1
...
def estimate_n_guesses(matrix, n_states, n_guesses_per_state=2,
    ...
    if singles_only:
        ...
        if matrix.method.level < 2 and matrix.type != "pp":
            # Adjustment for IP- and EA-ADC(0/1) calculations
            estimate = n_occ_a if matrix.type == "ip" else n_virt_a
