...
class ElectronicTransition(ElectronicStates):
    ...
    def transition_dipole_moment(self) -> np.ndarray:
        ...
        return np.array([self._transition_dipole_moment(i) for i in range(self.size)])
    ...
    def _transition_dipole_moment(self, state_n: int) -> np.ndarray:
        ...
        if self.property_method.level == 0:
            warnings.warn("ADC(0) transition dipole moments are known to be faulty in some cases.")
        dipole_integrals = self.operators.electric_dipole
        tdm = self._transition_dm(state_n)
        return np.array([product_trace(comp, tdm) for comp in dipole_integrals])
    ...
    def transition_dipole_moment_velocity(self) -> np.ndarray:
        ...
        computed states
        """
        return np.array([self._transition_dipole_moment_velocity(i) for i in range(self.size)])
    ...
    def _transition_dipole_moment_velocity(self, state_n: int) -> np.ndarray:
        ...
        if self.property_method.level == 0:
            warnings.warn(
                "ADC(0) transition velocity dipole moments are known to be faulty in some cases."
            )
    ...
    def transition_magnetic_dipole_moment(self, gauge_origin="origin") -> np.ndarray:
        ...
        return np.array(
            [
                self._transition_magnetic_dipole_moment(state_n=i, gauge_origin=gauge_origin)
                for i in range(self.size)
            ]
        )

    @cached_member_function(timer=_timer_name, separate_timings_by_args=False)
    def _transition_magnetic_dipole_moment(self, state_n: int, gauge_origin="origin") -> np.ndarray:
        """
        Computes the transition magnetic dipole moments for a single state
        """
        if self.property_method.level == 0:
            warnings.warn(
                "ADC(0) transition magnetic dipole moments are known to be faulty in some cases."
            )
        mag_dipole_integrals = self.operators.magnetic_dipole(gauge_origin)
        tdm = self._transition_dm(state_n)
        return np.array([product_trace(comp, tdm) for comp in mag_dipole_integrals])
    ...
    def _transition_quadrupole_moment(self, state_n: int, gauge_origin="origin"):
        ...
        if self.property_method.level == 0:
            warnings.warn(
                "ADC(0) transition quadrupole moments are known to be faulty in some cases."
            )
        quadrupole_integrals = self.operators.electric_quadrupole(gauge_origin)
        tdm = self._transition_dm(state_n)
        return np.array([[product_trace(q, tdm) for q in quad] for quad in quadrupole_integrals])

    def transition_quadrupole_moment_velocity(self, gauge_origin="origin") -> np.ndarray:
        """Array of transition quadrupole moments of all computed states"""
        return np.array(
            [
                self._transition_quadrupole_moment_velocity(state_n=i, gauge_origin=gauge_origin)
                for i in range(self.size)
            ]
        )

    @cached_member_function(timer=_timer_name, separate_timings_by_args=False)
    def _transition_quadrupole_moment_velocity(
        self, state_n: int, gauge_origin="origin"
    ) -> np.ndarray:
        """Compute the transition quadrupole moments for a single state"""
        if self.property_method.level == 0:
            warnings.warn(
                "ADC(0) transition velocity quadrupole moments are known "
    ...
    def _oscillator_strength(self, state_n: int) -> np.float64:
        ...
        return 2.0 / 3.0 * np.linalg.norm(tdm) ** 2 * np.abs(ev)

    @property
    def oscillator_strength_velocity(self) -> np.ndarray:
        """Array of oscillator strengths in velocity gauge of all computed states"""
        return np.array([self._oscillator_strength_velocity(i) for i in range(self.size)])
    ...
    def rotatory_strength(self) -> np.ndarray:
        ...
        """
        return np.array([self._rotatory_strength(i) for i in range(self.size)])
    ...
    @cached_member_function(timer=_timer_name, separate_timings_by_args=False)
    def _rotatory_strength_length(self, state_n: int, gauge_origin="origin") -> np.float64:
