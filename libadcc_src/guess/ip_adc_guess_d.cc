...
void transfer_elements(const list1d_t& o, const list1d_t& v, index_group_map_p2h& to,
...
size_t build_guesses(list_t::iterator& cur_guess, list_t::iterator end,
  ...
  if (spin == 0) {
    ...
    if (ig.size() == 1) {
      ...
      static const double coeff[3][3] = {// in case of ms == -1 (alpha ionization)
                                         // aaa   abb   bab
    ...
    if (ig.size() == 1) {
      ...
      static const double coeff[2][3] = {// in case of ms == -1 (alpha ionization)
                                         // aaa   abb   bab
                                         // and in case of ms == 1 (beta ionization)
                                         // bbb   baa   aba
                                         {0.0, -1.0,  1.0},    // doublet 1
    ...
    if (ig.size() == 3) {
      ...
      static const double coeff[1][3] = {// in case of ms == -1 (alpha ionization)
                                         // aaa   abb   bab
                                         // and in case of ms == 1 (beta ionization)
                                         // bbb   baa   aba
                                         {1.0, -1.0, -1.0}};  // quartet
